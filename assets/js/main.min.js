...
!(function (e, t) {
  ...
  $(function () {
    ...
    function b() {
      // ブレークポイントの変化でメジャリングやり直し
      v = $(window).width();
      var e = v < 768 ? 0 : v < 1024 ? 1 : v < 1280 ? 2 : 3;
      if (e !== y) {
        f(); // 幅配列 o[] を再計算
        y = e;
      }
      ...
      p =
        ...
        c.outerWidth(true) -
        (d.length !== 0 ? d.outerWidth(true) : 0) -
        // すべて表示できないケースでトグルを出すので、そのぶんも控除
        t.outerWidth(true);

      // 全項目の合計幅（o は累積幅配列。n は総項目数）
      var total = n > 0 ? o[n - 1] : 0;

      if (p >= total) {
        // すべて入る: ハンバーガーから可視側へ全戻し
        a.children().appendTo(i);
        h = n;
        t.attr("count", 0).addClass("hidden");
      } else {
        // 1つでも入らない: すべてハンバーガーへ
        i.children().appendTo(a);
        h = 0;
        t.attr("count", n).removeClass("hidden");
      }
    }
